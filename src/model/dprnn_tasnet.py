...
class Encoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, n_speakers):
        ...
        self.res_conv = nn.Conv1d(in_channels=hidden_dim, out_channels=hidden_dim*n_speakers, kernel_size=1)
...
class Decoder(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.dconv1d_block = nn.Sequential(
            nn.ConvTranspose1d(in_channels=input_dim, out_channels=input_dim, kernel_size=1),

            nn.PReLU(),
            nn.GroupNorm(num_groups=1, num_channels=input_dim), # TODO: add cumulative layer norm, because the task is causal

            nn.ConvTranspose1d(in_channels=input_dim, out_channels=input_dim, kernel_size=3, padding=1, groups=hidden_dim),

            nn.PReLU(),
            nn.GroupNorm(num_groups=1, num_channels=input_dim), # TODO: add cumulative layer norm, because the task is causal
            )

        self.out_conv = nn.ConvTranspose1d(in_channels=input_dim, out_channels=hidden_dim, kernel_size=1)
...
class OverlapAdd(nn.Module):
    ...
    def forward(self, x):
        ...
        return x.contiguous().view(batch, features, output_length)[:, :, :self.n_frames]
...
class DPRNN_TasNet(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(input_dim=input_dim, hidden_dim=enc_hidden_dim, n_speakers=n_speakers)

        self.sep_start = nn.Sequential(
            nn.GroupNorm(num_groups=1, num_channels=enc_hidden_dim),
            nn.Conv1d(in_channels=enc_hidden_dim, out_channels=enc_hidden_dim, kernel_size=1),
        )
