...
class TFARUnit(nn.Module):
    ...
    """
    def __init__(self, channels_m: int, channels_n: int, dimensionality_type: Literal['2D', '1D'] = '2D'):
        super().__init__()
        ...
        self.w1_pathway = nn.Sequential(
            conv_class(channels_n, channels_n, kernel_size=4, stride=2, padding=1, groups=channels_n), # depthwise conv (same size)
            GlobalLayerNorm(channels_n), # gLN
            nn.Sigmoid(),
        )
        self.w2_pathway = nn.Sequential(
            conv_class(channels_m, channels_m, kernel_size=4, stride=2, padding=1, groups=channels_m), # depthwise conv (same size)
            GlobalLayerNorm(channels_m), # gLN
        )
        self.w3_pathway = nn.Sequential(
            conv_class(channels_n, channels_n, kernel_size=3, padding=1, groups=channels_n), # depthwise conv (same size)
            GlobalLayerNorm(channels_n), # gLN
        )
    ...
    def forward(self, m: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
        ...
        w1_up = functional.interpolate(w1, size=interpolate_shape, mode='bilinear', align_corners=False)
        w3_up = functional.interpolate(w3, size=interpolate_shape, mode='bilinear', align_corners=False)
        out = w1_up * w2 + w3_up
...
class ReconstructionBlock(nn.Module):
    ...
    """
    def __init__(self, in_channels: int, out_channels: int, upsample_units: int = 2, dimensionality_type: Literal['2D', '1D'] = '2D'):
        super().__init__()
        ...
        for i in range(upsample_units):
            self.g_fuse_layers.append(TFARUnit(in_channels_m, in_channels_n, dimensionality_type=dimensionality_type))

            in_channels_m_next = in_channels_m // 2  # assuming downsample factor of 2 in compressor

            if i != 0:
                self.residual_fuse_layers.append(TFARUnit(in_channels_m, in_channels_m_next, dimensionality_type=dimensionality_type))
        ...
        self.dimension_upsampling_layer = conv_class(in_channels, out_channels, kernel_size=1)  # Restore to original channels
