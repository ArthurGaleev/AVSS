...
from src.metrics.utils import stoi, pesq, si_sdr_i, si_snr_i
import argparse
...
def calculate_metrics(gt_s1_path, gt_s2_path, pred_s1_path, pred_s2_path):
    ...
    assert gt_s1_files == gt_s2_files == pred_s1_files == pred_s2_files, "File lists must match"
    
    for file in gt_s1_files:
        ...
        references = np.stack([gt_s1, gt_s2], axis=0)
        mix = np.stack([gt_s1 + gt_s2, gt_s1 + gt_s2], axis=0)
        estimates = np.stack([pred_s1, pred_s2], axis=0)
        
        # Calculate metrics
        stoi_score = stoi(references, estimates, sr1)
        pesq_score = pesq(references, estimates, sr1)
        si_sdr_i_score = si_sdr_i(references, estimates)
        si_snr_i_score = si_snr_i(references, estimates)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gt_s1", required=True, help="Path to ground truth s1 directory.")
    parser.add_argument("--gt_s2", required=True, help="Path to ground truth s2 directory.")
    parser.add_argument("--pred_s1", required=True, help="Path to predicted s1 directory.")
    parser.add_argument("--pred_s2", required=True, help="Path to predicted s2 directory.")
