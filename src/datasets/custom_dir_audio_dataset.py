...
class CustomDirAudioDataset(BaseDataset):
    def __init__(self, audio_mix_dir,audio_first_dir, audio_second_dir, *args, **kwargs):
        data = []
...
class YandexDownload(CustomDirAudioDataset):
    def __init__(self, download_name="dla_dataset_small_a", data_dir=None, part="train",*args, **kwargs):
        assert download_name in YANDEX_URL
        if data_dir is None:
            data_dir = ROOT_PATH / "data" / "datasets"
        print(data_dir/download_name/part)
        if not (data_dir/download_name/"audio"/part).exists():
            timer=datetime.now()
            data_dir.mkdir(exist_ok=True, parents=True)
            download_info=YANDEX_URL[download_name]
            final_url = download_info["base_url"] + urlencode(dict(public_key=download_info["public_key"]))
            response = requests.get(final_url)
            download_url = response.json()['href']
            download_response = requests.get(download_url)
            zip = zipfile.ZipFile(io.BytesIO(download_response.content))
            zip.extractall(data_dir)
            print("Total Seconds: ", (datetime.now()-timer).total_seconds())
        data_dir=data_dir/download_name/"audio"/part
        super().__init__(data_dir/"mix", data_dir/"s1", data_dir/"s2", *args, **kwargs)