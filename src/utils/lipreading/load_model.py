...
def update_args(config_path):
    ...
    if args.get('tcn_kernel_size', ''):
        args['tcn_options'] = { 'num_layers': args['tcn_num_layers'],
    ...
    if args.get('densetcn_block_config', ''):
        args['densetcn_options'] = {'block_config': args['densetcn_block_config'],
...
def load_lipreading_model(model_name, logger, device):
    ...
    update_args(ROOT_PATH / "src" / "utils" / "lipreading" / "configs"/ f"lrw_{model_name}.json")
    ...
    model = load_model(model_name, model, device=device, allow_size_mismatch=args['allow_size_mismatch'])
