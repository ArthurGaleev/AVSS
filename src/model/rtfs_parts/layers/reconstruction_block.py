...
class TFARUnit(nn.Module):
    ...
    """
    def __init__(self, in_channels: int, dimensionality_type: Literal['2D', '1D'] = '2D'):
        super().__init__()
        ...
        self.w1_pathway = nn.Sequential(
            conv_class(in_channels, in_channels, kernel_size=4, stride=2, padding=1, groups=in_channels), # depthwise conv (same size)
        ...
        self.w3_pathway = nn.Sequential(
            conv_class(in_channels, in_channels, kernel_size=3, padding=1, groups=in_channels), # depthwise conv (same size)
            GlobalLayerNorm(in_channels), # gLN
        )
    ...
    def forward(self, m: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
        ...
        w1_up = functional.interpolate(w1, size=interpolate_shape, mode='nearest', align_corners=False)
        w3_up = functional.interpolate(w3, size=interpolate_shape, mode='nearest', align_corners=False)
        out = w1_up * w2 + w3_up
...
class ReconstructionBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, upsample_units: int = 2, dimensionality_type: Literal['2D', '1D'] = '2D'):
        ...
        for _ in range(upsample_units):
            ...
            self.g_fuse_layers.append(TFARUnit(in_channels, dimensionality_type=dimensionality_type))

        for _ in range(upsample_units - 1):
            # When reconstructing, we go from coarsest to finest
            self.residual_fuse_layers.append(TFARUnit(in_channels, dimensionality_type=dimensionality_type))
    ...
    def forward(self, A_i: List[torch.Tensor], A_G: torch.Tensor) -> torch.Tensor:
        ...
        for layer, A_finer, A_q in zip(self.residual_fuse_layers, A_prime[-2::-1], A_i[-2::-1]):
